...
export const TypeIcon = ({ type }) => {
  if (!type) {
    return (
      <Popup content={UNDEFINED} trigger={<i className='fas fa-question' />} />
    )
  }
  if (type === BUG) {
    return <Popup content={BUG} trigger={<i className='fas fa-bug' />} />
  }
  if (type === FEATURE) {
    return <Popup content={FEATURE} trigger={<i className='fas fa-star' />} />
  } else {
